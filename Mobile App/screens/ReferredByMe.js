...
export default function ReferredByMe(dummy) {
  const BACKEND_URL = process.env.BACKEND_URL || "http://170.187.254.215:4000";
  ...
  const fetch_peoples = async () => {
    ...
    let res = await fetch(`${BACKEND_URL}/api/references/to`, requestOptions);
      ...
      <View style={styles.personContainer}>
        ...
        >
          ...
          <Text style={styles.time}>
            {new Date(item.referenceDateTime).toLocaleTimeString()}
          </Text>
          <Text style={styles.date}>
            {new Date(item.referenceDateTime).toLocaleDateString()}
          </Text>
          <Image
